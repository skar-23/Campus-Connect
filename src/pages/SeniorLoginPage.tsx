...
const SeniorLoginPage: React.FC = () => {
  ...
  const handleLogin = async (values: LoginFormValues) => {
    setIsLoading(true);
    try {
      // This page correctly identifies itself as a 'senior' login.
      await signIn(values.email, values.password, 'senior');
    } catch (error: any) {
    ...
    <div className="min-h-screen flex flex-col bg-background">
      ...
      <main className="flex-grow flex flex-col items-center justify-center container py-12">
        <Card className="w-full max-w-md">
          <CardHeader>
            <CardTitle className="text-2xl text-center">Senior Login</CardTitle>
          </CardHeader>
          <CardContent>
            <Form {...form}>
              <form onSubmit={form.handleSubmit(handleLogin)} className="space-y-6">
                ...
                <div className="flex justify-end">
                  <Button asChild variant="link" className="px-0">
                    <Link to="/senior-forgot-password">Forgot Password?</Link>
                  </Button>
                </div>
                ...
                <div className="text-center text-sm text-muted-foreground">
                  ...
                  <Link to="/senior-signup" className="text-primary hover:underline">
                    Signup
                  </Link>
                </div>
              </form>
